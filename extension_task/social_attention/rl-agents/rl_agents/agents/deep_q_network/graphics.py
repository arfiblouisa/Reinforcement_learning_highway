...
class DQNGraphics(object):
        ...
        try:
            ...
            for head in range(list(cls.v_attention.values())[0].shape[0]):
                ...
                for vehicle, attention in cls.v_attention.items():
                    ...
                    if vehicle is agent.env.unwrapped.vehicle:
                        pygame.draw.circle(attention_surface, color,
                                           sim_surface.vec2pix(agent.env.unwrapped.vehicle.position),
    ...
    def compute_vehicles_attention(cls, agent, state):
        ...
        mask = mask.squeeze()
        v_attention = {}
        obs_type = agent.env.unwrapped.observation_type
        ...
        for v_index in range(state.shape[0]):
            ...
            vehicle = min(agent.env.unwrapped.road.vehicles, key=lambda v: np.linalg.norm(v.position - v_position))
